...
def checkFolder():
    ...
    if not os.path.isdir("pjs"):
        askAuthUser = input(
            'Se ha creado la carpeta "pjs" para almacenar los personajes'
        )
        os.makedirs("pjs")
    return os.listdir(PATH_FOLDER)


# LIST OF PERSONAJES IN ARRAY
...
def showPjInfo(pjName):
    ...
    if elegido not in ABNE:
        ...
        if isinstance(pj[elegido], dict):
            showTable(pj[elegido])
        if input("Open in new terminal? y/any \n").lower() == "y":
            show_table_in_new_terminal(pj[elegido])
...
def menu():
    ...
    links = {
        "Manual": "https://drive.google.com/drive/u/0/folders/1cLwQJNBNFOMCxDp7t_fC4DHqz9kdyJUA",
                ...
                if not isinstance(pj[primer][segundo], dict):
                    ...
                    while True:
                        ...
                        if not isinstance(pj[primer][segundo][tercer], dict):
                            pj[primer][segundo][tercer] = modificarString(
                                pj[primer][segundo][tercer]
                            )
                            break
                        else:
...
def addPj():
    ...
    if input("Hechizo? y/any \n").lower() == "y":
        hechizos = addNew(INFO_CON_NOMBRE["hechizos"], True)

    # Habilidades
    if input("Habilidad? y/any \n").lower() == "y":
        habilidades = addNew(INFO_CON_NOMBRE["habilidades"], True)
    ...
    if isinstance(data, dict):
        ...
        subprocess.run(
            [
                ...
                "bash",
                "-c",
                f'echo "{table}" && read -p "Press any key to exit..."',
            ]
        )
    else:
        print("Invalid data format. Expected dictionary.")
